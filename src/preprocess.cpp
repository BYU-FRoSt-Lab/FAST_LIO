...
Date: [09-2024]
Description: Added in functionality for handling an OS0 or OS1 
...
void Preprocess::semantic_oust128_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  // RCLCPP_INFO(rclcpp::get_logger("liosam_debugger"), "Processing Semantic Ouster 128 Message");
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  pcl::PointCloud<ouster_ros::SemanticPoint128> pl_orig;
...
void Preprocess::oust128_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  pcl::PointCloud<ouster_ros::Point128> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.size();
  pl_corn.reserve(plsize);
  pl_surf.reserve(plsize);
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  pl_corn.clear();
  pl_full.clear();
  pcl::PointCloud<ouster_ros::Point64> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.size();
  pl_corn.reserve(plsize);
  pl_surf.reserve(plsize);
